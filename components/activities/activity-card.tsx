...
export function ActivityCard({
  ...
  return (
    <article
      ...
      className={cn(
        "glass-panel rounded-xl border border-border/50 p-3 shadow-card transition-all duration-150",
    ...
    >
      <div className="flex items-start gap-2">
        <button
          type="button"
          className="mt-0.5 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-lg text-tertiary transition hover:bg-accent/[0.06] hover:text-secondary"
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <h3 className="text-[13px] font-semibold leading-snug text-foreground">{activity.title}</h3>
            <span className={cn("rounded-md border px-1.5 py-0.5 text-[10px] font-bold uppercase", PRIORITY_CLASS[activity.priority])}>
              {PRIORITY_LABEL[activity.priority]}
            </span>
          </div>

          {activity.description && (
            <p className="mt-2 line-clamp-2 rounded-lg bg-muted/60 px-2.5 py-1.5 text-[11px] leading-relaxed text-secondary/85">
              {activity.description}
            </p>
          )}
          ...
          <div className="mt-2.5 space-y-1.5">
            ...
            {activity.lead && (
              <div className="rounded-lg border border-border/45 bg-surface/50 px-2.5 py-1.5">
      ...
      {!isOverlay && (
        <div className="mt-2.5 flex justify-end gap-1.5 border-t border-border/35 pt-2.5">
          <button
            type="button"
